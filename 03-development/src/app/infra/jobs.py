...
# ---------------------------------------------------------------------------
# [FR-78] Batch import of knowledge entries.
#
# SRS.md line 176 (FR-78):
#     批量匯入 >10 筆知識：非同步 enqueue 所有 chunks（首塊不阻塞同步
#     embedding 等待）；單筆 < 50ms；回傳 sync_wait=False 的結構化結果。
#
# SAD §Module: jobs.py contract + TEST_SPEC.md line 1583 pin:
#   - batch_import_knowledge(entries: list[dict], is_batch: bool)
#       -> BatchImportResult(sync_wait, entry_count, enqueued_count)
# ---------------------------------------------------------------------------

@dataclass(frozen=True)
class BatchImportResult:
    """[FR-78] Structured outcome of a batch knowledge-import invocation.

    Attributes:
        sync_wait: ``True`` iff the caller should expect a synchronous
            embedding wait for at least one chunk (single-entry path).
            ``False`` for batch mode where ALL chunks are enqueued
            asynchronously and the caller does not block.
        entry_count: Number of input entries submitted to the batch.
        enqueued_count: Number of chunks successfully enqueued for async
            embedding (equal to ``entry_count`` for the happy path).
    """
...
def batch_import_knowledge(
    entries: list[dict], is_batch: bool
) -> BatchImportResult:
    """[FR-78] Enqueue a list of knowledge entries for async embedding.

    When ``is_batch=True`` ALL chunks are enqueued without a synchronous
    embedding wait (returns ``sync_wait=False``). When ``is_batch=False``
    the legacy single-entry path is used; ``sync_wait`` reflects whether
    the first chunk was embedded inline.

    The function is intentionally synchronous at the entry-point — actual
    embedding work is dispatched via :func:`enqueue_embedding_job`. Per-entry
    processing stays well under the NP-06 50 ms SLA because the only work
    performed here is dict validation + enqueue (no network I/O).
    """
    if not entries:
        return BatchImportResult(
            sync_wait=not is_batch, entry_count=0, enqueued_count=0
        )
    enqueued = 0
    for entry in entries:
        # Lightweight validation — ensures each entry has the minimum
        # fields the embedding pipeline requires. The async worker picks
        # up the chunk via enqueue_embedding_job.
        if "title" in entry and "content" in entry:
            enqueue_embedding_job(
                EmbeddingJob(
                    id=str(entry.get("title", "")),
                    text=str(entry.get("content", "")),
                    model=str(entry.get("model", "text-embedding-3-small")),
                )
            )
            enqueued += 1
    return BatchImportResult(
